...
(() => {
        ...
        return function() {
          ...
          class C {
            ...
            _createElement() {
              ...
              this.$cell = n({ attrs: __spreadValues({ "data-year": t2, "data-month": i2, "data-date": s2 }, a2) }), this.$cell.adpCell = this;
            }
            _getClassName() {
              var e2;
  ...
  var INITIAL_DATE = new Date(2e3, 0, 1);
  var DATE_TODAY = /* @__PURE__ */ new Date();
  var BOX_WIDTH = "w-[min(0.5rem,1.25vw)]";
  var BOX_COLOR = "bg-red-600";
  var LOADING_TIME = 2e3;
      ...
      for (let age = -1; age <= LIFESPAN; age++) {
        ...
        for (let week = 0; week < WEEKS_IN_YEAR; week++) {
          if (age === -1) {
            ...
            box.dataset["week"] = (1 + age * WEEKS_IN_YEAR + week).toString();
            row.append(box);
          }
        }
        canvas.append(row);
  ...
  function getElapasedWeeks(selectedDate) {
    const elapsedYears = DATE_TODAY.getUTCFullYear() - selectedDate.getUTCFullYear();
    const isThisYearsBirthdayPassed = DATE_TODAY.getTime() > new Date(
    ...
    const elapsedDaysSinceLastBirthday = Math.floor(
      (DATE_TODAY.getTime() - dateOfLastBirthday.getTime()) / (1e3 * 60 * 60 * 24)
    );
    const elapsedWeeks = elapsedYears * WEEKS_IN_YEAR + Math.floor(elapsedDaysSinceLastBirthday / 7);
    return elapsedWeeks;
  }
  var DatePicker = {
    init: (boxes, datepicker) => {
      const dp = new import_air_datepicker.default(`#${datepicker.id}`, {
        locale: {
          days: [
            "Sunday",
            "Monday",
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    !isPageVisited ? loader_default.init(loader) : loader_default.show(elems);
    const canvas = document.getElementById("canvas");
    if (canvas === null) {
      console.error(`Missing element with id 'canvas'`);
      return;
    }
    canvas_default.init(canvas);
    ...
    datepicker_default.init(boxes, datepicker);
    if (!isPageVisited) {
      setTimeout(() => {
        loader_default.end(loader);
        loader_default.show(elems);
      }, LOADING_TIME);
    }
  });
})();
